...
type Language = 'en' | 'ro'
type Tab = 'documents' | 'doc-processing' | 'knowledge-graph' | 'retrieval' | 'api' | 'settings' | 'evaluation'
...
interface SettingsState {
  ...
  // Knowledge Graph Insights settings
  enableKGInsights: boolean
...
const useSettingsStoreBase = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      // Knowledge Graph Insights settings
      enableKGInsights: false,
      setEnableKGInsights: (enable: boolean) => set({ enableKGInsights: enable }),
      ...
      updateQuerySettings: (settings: Partial<QueryRequest>) => {
        // Allow history_turns to be configurable (0-3 turns for follow-up questions)
        set((state) => ({
