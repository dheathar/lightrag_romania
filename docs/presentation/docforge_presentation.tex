...
\begin{frame}{Try These Questions Now}
  ...
  \begin{enumerate}\setlength\itemsep{10pt}
    \item \textit{``What were the main barriers to implementation\\
          shared by POCU, POIM and POR?''}\\
          {\small\textcolor{coral}{\textbf{Shows:}} \textcolor{midgray}{cross-programme synthesis — the graph connects themes
          across all 5 reports that no single document contains \quad
          \texttt{/global}}}

    \item \textit{``What specific employment targets did POCU set\\
          and were they achieved?''}\\
          {\small\textcolor{coral}{\textbf{Shows:}} \textcolor{midgray}{precision retrieval — exact indicators, numbers and
          targets pulled from within a specific programme \quad
          \texttt{/local}}}

    \item \textit{``Compare the monitoring weaknesses identified\\
          in POIM and POR.''}\\
          {\small\textcolor{coral}{\textbf{Shows:}} \textcolor{midgray}{cross-document comparison — two separate programmes
          analysed side-by-side with cited sources from each \quad
          \texttt{/hybrid}}}

    \item \textit{``Care sunt principalele recomandări pentru\\
          îmbunătățirea absorbției fondurilor în POCU și POIM?''}\\
          {\small\textcolor{coral}{\textbf{Shows:}} \textcolor{midgray}{multilingual — Romanian question, Romanian answer,
          same knowledge base \quad custom prompt: \texttt{Answer in Romanian.}}}
  \end{enumerate}
\end{frame}
  ...
  \begin{columns}[t]
    ...
    \begin{column}{0.46\textwidth}
      \begin{alertblock}{\faGlobe\ Try it now}
        \centering\medskip
        \textbf{doclens.labor-innovation.com}\\[6pt]
        {\small No login required.\\
         5 reports loaded for demo.}
        \medskip
      \end{alertblock}
      \vspace{0.5em}
