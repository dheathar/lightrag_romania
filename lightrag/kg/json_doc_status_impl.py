...
class JsonDocStatusStorage(DocStatusStorage):
    ...
    async def get_by_ids(self, ids: list[str]) -> list[dict[str, Any]]:
        ...
        async with self._storage_lock:
            for id in ids:
                ...
                if data:
                    ordered_results.append(copy.deepcopy(data))
                else:
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        if self._storage_lock is None:
            raise StorageNotInitializedError("JsonDocStatusStorage")
        async with self._storage_lock:
            # Deep copy input to avoid mutating caller's data and ensure isolation
            for doc_id, doc_data in data.items():
                safe_data = copy.deepcopy(doc_data)
                if "chunks_list" not in safe_data:
                    safe_data["chunks_list"] = []
                self._data[doc_id] = safe_data
            await set_all_update_flags(self.namespace, workspace=self.workspace)

        # Persist AFTER releasing the lock to avoid re-entrant lock error
        # (index_done_callback acquires the same lock internally)
    ...
    async def get_docs_paginated(
        ...
        async with self._storage_lock:
            for doc_id, doc_data in self._data.items():
                ...
                try:
                    # Prepare document data (deep copy for nested dict safety)
                    data = copy.deepcopy(doc_data)
                    data.pop("content", None)
                    if "file_path" not in data:
                        data["file_path"] = "no-file-path"
                    if "metadata" not in data:
        ...
        async with self._storage_lock:
            for doc_id, doc_data in self._data.items():
                if doc_data.get("file_path") == file_path:
                    # Return deep copy, consistent with get_by_ids method
