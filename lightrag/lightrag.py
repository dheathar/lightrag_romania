...
from lightrag.chunking import (
    ChunkingMethod,
    create_semantic_chunking_func,
    create_hybrid_chunking_func,
)
...
class LightRAG:
    ...
    chunking_method: str = field(
        default=os.getenv("CHUNKING_METHOD", ChunkingMethod.TOKEN_SIZE.value)
    )
    """Chunking method to use: TOKEN_SIZE, SEMANTIC, or HYBRID.

    - TOKEN_SIZE: Fixed-size token-based chunking (default, fastest)
    - SEMANTIC: Embedding-based semantic similarity chunking (better coherence, slower)
    - HYBRID: Token-based chunking with semantic boundary validation
    """

    semantic_similarity_threshold: float = field(
        default=float(os.getenv("SEMANTIC_SIMILARITY_THRESHOLD", "0.8"))
    )
    """Percentile threshold for semantic breakpoint detection (0.0-1.0).

    Higher values = fewer, larger chunks. Default 0.8 (80th percentile).
    Only used when chunking_method is SEMANTIC or HYBRID.
    """

    semantic_min_chunk_size: int = field(
        default=int(os.getenv("SEMANTIC_MIN_CHUNK_SIZE", "100"))
    )
    """Minimum tokens per semantic chunk.

    Smaller chunks are merged with neighbors. Only used when chunking_method is SEMANTIC or HYBRID.
    """

    semantic_max_tokens: int = field(
        default=int(os.getenv("SEMANTIC_MAX_TOKENS", "100000"))
    )
    """Maximum document tokens before falling back to token-based chunking.

    Documents larger than this will use token-based chunking regardless of chunking_method.
    """
    ...
    def __post_init__(self):
        ...
        # Step 3: Configure chunking function based on chunking_method
        # Only override if chunking_func is the default (user hasn't provided custom func)
        if self.chunking_func is chunking_by_token_size:
            method = self.chunking_method.upper()
            if method == ChunkingMethod.SEMANTIC.value:
                if self.embedding_func is None:
                    logger.warning(
                        "SEMANTIC chunking requires embedding_func, falling back to TOKEN_SIZE"
                    )
                else:
                    self.chunking_func = create_semantic_chunking_func(
                        embedding_func=self.embedding_func,
                        similarity_threshold=self.semantic_similarity_threshold,
                        min_chunk_tokens=self.semantic_min_chunk_size,
                        max_tokens_for_semantic=self.semantic_max_tokens,
                    )
                    logger.info(
                        f"Using SEMANTIC chunking (threshold={self.semantic_similarity_threshold})"
                    )
            elif method == ChunkingMethod.HYBRID.value:
                if self.embedding_func is None:
                    logger.warning(
                        "HYBRID chunking requires embedding_func, falling back to TOKEN_SIZE"
                    )
                else:
                    self.chunking_func = create_hybrid_chunking_func(
                        embedding_func=self.embedding_func,
                        similarity_threshold=self.semantic_similarity_threshold,
                        min_chunk_tokens=self.semantic_min_chunk_size,
                        max_tokens_for_semantic=self.semantic_max_tokens,
                    ...
                    logger.info(
                        f"Using HYBRID chunking (threshold={self.semantic_similarity_threshold})"
                    )
            elif method != ChunkingMethod.TOKEN_SIZE.value:
                logger.warning(
                    f"Unknown chunking_method '{self.chunking_method}', using TOKEN_SIZE"
                )
    ...
    async def _validate_and_fix_document_consistency(
        ...
        if docs_to_reset:
            ...
            # Only update in-memory state AFTER storage upsert succeeds
            for doc_id in docs_to_reset:
                if doc_id in to_process_docs:
                    to_process_docs[doc_id].status = DocStatus.PENDING
                                ...
                                self.doc_status.upsert(
                                    {
                                        doc_id: {
                                            ...
                                            "metadata": {
                                                **(status_doc.metadata if isinstance(status_doc.metadata, dict) else {}),
