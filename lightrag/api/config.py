...
def parse_args() -> argparse.Namespace:
    ...
    args.chunking_method = get_env_value("CHUNKING_METHOD", "TOKEN_SIZE")
    args.semantic_similarity_threshold = get_env_value(
        "SEMANTIC_SIMILARITY_THRESHOLD", 0.8, float
    )
    args.semantic_min_chunk_size = get_env_value("SEMANTIC_MIN_CHUNK_SIZE", 100, int)
    args.semantic_max_tokens = get_env_value("SEMANTIC_MAX_TOKENS", 100000, int)
    ...
    # Vision model configuration (for describing figures/charts in PDFs)
    args.vision_enabled = get_env_value("VISION_ENABLED", False, bool)
    args.vision_model = get_env_value("VISION_MODEL", "")
    args.vision_api_key = get_env_value("VISION_API_KEY", "")
    args.vision_base_url = get_env_value("VISION_BASE_URL", "")
    args.vision_prompt = get_env_value(
        "VISION_PROMPT",
        "Describe this figure, chart, or diagram in detail. "
        "Include all data values, labels, trends, and relationships visible. "
        "Be precise and factual.",
    )
    args.docling_images_scale = get_env_value("DOCLING_IMAGES_SCALE", 2.0, float)
    args.max_figures_per_doc = get_env_value("MAX_FIGURES_PER_DOC", 20, int)

    # Vision OCR configuration (for healing PDFs with broken font glyph encoding)
    args.vision_ocr_prompt = get_env_value(
        "VISION_OCR_PROMPT",
    ...
    args.max_vision_ocr_pages = get_env_value("MAX_VISION_OCR_PAGES", 50, int)
