...
def create_app(args):
    ...
    app.include_router(
        create_config_routes(args, api_key, rerank_model_func is not None)
    )

    # Add evaluation routes (RAGAS)
    # Use LIGHTRAG_API_URL env var if set, otherwise construct from args
    # Note: 0.0.0.0 is a bind address, not a connectable address - use localhost instead
    rag_api_url = os.getenv("LIGHTRAG_API_URL")
    if not rag_api_url:
        host = "localhost" if args.host == "0.0.0.0" else args.host
        rag_api_url = f"http://{host}:{args.port}"
    app.include_router(create_evaluation_routes(api_key, rag_api_url))
