...
TOTAL_TIMEOUT_SECONDS = 600.0  # Must be >= READ_TIMEOUT to avoid premature cutoff
...
class RAGEvaluator:
    ...
    def __init__(
        ...
        Args:
            ...
            progress_callback: Optional callback function(completed: int, total: int)
                             Called after each test completes to report progress
        ...
        # Store progress callback for WebUI integration
    ...
    async def evaluate_single_case(
        ...
        Args:
            ...
            total_count: Total number of test cases (for progress reporting)
        ...
        async with rag_semaphore:
            ...
            try:
                ...
                # Report progress to callback (for WebUI updates)
                if self.progress_callback:
                    self.progress_callback(progress_counter["completed"], total_count)
                return {
                    "test_number": idx,
                    "question": question,
                    "error": str(e),
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        logger.info(
            "Avg Time/Test:  %.2f seconds",
            elapsed_time / len(results) if results else 0,
        )
